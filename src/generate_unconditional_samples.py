    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        if debug:
            writer = tf.summary.FileWriter(
                '/media/eilab/humor_gen/caras_humor/logs')
...
def sample_combined_models_with_seed(
    model_name='117M',
    run_name1='kdrama_finetune',
    run_name2='dnd_bios2',
    seed=None,
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        if False:
            ...
            all_text = ''
            while nsamples == 0 or generated < nsamples:
                out, mx = sess.run(output, feed_dict={context: [context_tokens for _ in range(batch_size)]})
...
def print_combined_logits(
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        log_dir = '/media/eilab/humor_gen/caras_humor/logs'
        while nsamples == 0 or generated < nsamples:
